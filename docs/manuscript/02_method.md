...
The two record types are stated as [@def:line-entry] and [@def:shared-token]. The declaration ships eight line entries and eight shared token. The wrapper's own entry lives beside `LINE_SET` rather than inside it, because it is not an instrument of its own; its `working_position` continues the sequence so that appending it still yields a contiguous run.
